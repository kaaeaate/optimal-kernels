...
class kernel_block_15(nn.Module):
    def __init__(self,ch_in,ch_out,kk2_size):
        super(kernel_block_15, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_out, kernel_size=kk2_size, stride=1, padding=int(kk2_size/2)),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True)
        )
    def forward(self,x):
...
class first_block(nn.Module):
    def __init__(self,ch_in, ch_out, k_size):
        super(first_block,self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_out, kernel_size=3,stride=1,padding=1,bias=True),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch_out, ch_out, kernel_size=k_size,stride=1,padding=int(k_size/2),bias=True),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True)
        )
    def forward(self,x):
...
class Hybrid(nn.Module):
    def __init__(self,img_ch=3,output_ch=3, k_size=3, kk2_size=15):
        super(Hybrid,self).__init__()
        
        self.Maxpool = nn.MaxPool2d(kernel_size=2,stride=2)

        self.Conv1 = first_block(ch_in=img_ch,ch_out=64, k_size=k_size)
...
#         self.Att_out = nn.Conv2d(64,3,kernel_size=1,stride=1,padding=0)
        #-----------
        self.Conv_upd_x1 = kernel_block_15(ch_in=img_ch,ch_out=64, kk2_size=kk2_size)
        #-----------
